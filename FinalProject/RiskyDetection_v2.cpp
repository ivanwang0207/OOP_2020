...
private:
    map<string,tuple<double,double>> RiskTable; //工作項目, (危險指數, 危險範圍)
public:
    virtual void ReadDataBase(){   
        ...
        for(int i = 1; i!=matrix.size(); i++){
           ...
           double idx = atof(matrix[i][1].c_str()); 
           double range = atof(matrix[i][2].c_str());
           RiskTable.insert(pair<string,tuple<double,double>>(item,make_tuple(idx,range))); 
        }
    };
...
private:
    map<string,double> RiskTable; //危險區域, 危險指數
public:
    virtual void ReadDataBase(){
        ...
        vector<vector<string>> matrix;
        fstream file;
        file.open("RegionRiskyTable.csv");
        string line;
        while (getline(file, line,'\n'))  //讀檔讀到跳行字元
        {
            istringstream templine(line); // string 轉換成 stream
        ...
        for(int i = 1; i!=matrix.size(); i++){
           string item = matrix[i][0];
           double idx = atof(matrix[i][1].c_str()); 
           RiskTable.insert(pair<string,double>(item,idx)); 
        }
    };
...
public:
    ...
    virtual void ReadDataBase(){ //之後要用database寫入
        ...
        for(int i = 1; i!=matrix.size(); i++){
           ...
           string workingitem = matrix[i][1];
           double x = atof(matrix[i][2].c_str());
           double y = atof(matrix[i][3].c_str());
           int age = atoi(matrix[i][4].c_str()); 
           int senior = atoi(matrix[i][5].c_str()); 
           workers.push_back(make_tuple(name,workingitem,x,y,age,senior));
        }
...
public:
    ...
    virtual void ReadDataBase(){ //之後要用database寫入
        ...
        for(int i = 1; i!=matrix.size(); i++){
           string name = matrix[i][0];
           double x = atof(matrix[i][1].c_str());
           double y = atof(matrix[i][2].c_str());
           double w = atof(matrix[i][3].c_str()); 
           double h = atof(matrix[i][4].c_str()); 
           Regions.push_back(make_tuple(name,x,y,w,h));
        }
...
public:
    ...
    virtual double getIndex(){return index;}
    virtual ~Worker(){
        delete RiskTable;
...
public:
    ...
    /*
    virtual GetWorkerRiskyTable* makeWorkerRiskyTable(string TableName) const{
        return new GetWorkerRiskyTable();
    }*/
};
    ...
    void StartDetection(){
        ...
        for(auto w_iter = v_workers.begin(); w_iter != v_workers.end(); ++w_iter){            
            ...
            for(auto r_iter = v_regions.begin(); r_iter != v_regions.end(); ++r_iter ){                                             
                ...
                if (dx < dw / 2 && dy < dh / 2) {
                    cout << (*w_iter)->getName() << " location = (" << get<0>((*w_iter)->getLocation()) << "," << get<1>((*w_iter)->getLocation()) << ")"<<" is inside " << (*r_iter)->getName() << "\nRisky range = (min("<<get<0>(get<0>((*r_iter)->getRange())) << "," << get<1>(get<0>((*r_iter)->getRange())) << "),max(" << get<0>(get<1>((*r_iter)->getRange())) << "," << get<1>(get<1>((*r_iter)->getRange())) << "))\nRisky index is: " << (*w_iter)->getIndex() * (*r_iter)->getIndex() << endl;
                    ...
                    cout << (*w_iter)->getName() << " location = (" << get<0>((*w_iter)->getLocation()) << "," << get<1>((*w_iter)->getLocation()) << ")"<<" is approaching " << (*r_iter)->getName() << "\nRisky range = (min("<<get<0>(get<0>((*r_iter)->getRange())) << "," << get<1>(get<0>((*r_iter)->getRange())) << "),max(" << get<0>(get<1>((*r_iter)->getRange())) << "," << get<1>(get<1>((*r_iter)->getRange())) << "))\nRisky index is: " << (*w_iter)->getIndex() * (*r_iter)->getIndex() << endl;
