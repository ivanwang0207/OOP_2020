...
For the example above, your program should return -2. If the matrix is not a square matrix, return -1. 
The maximum size of strArr will be a 6x6 matrix. The determinant will always be an integer.
*/
...
int MatrixDeterminant(string[], int) ;
int third_deg(vector < vector<int> >&);
int base_est(vector < vector<int> >& );
...
int calculator(vector < vector<int> > currentMatrix)
{
	...
	if (currentMatrix.size() == 2)
        return base_est(currentMatrix);

    else{
        return third_deg(currentMatrix);
    }
}

int third_deg(vector < vector<int> > &cmat){
    int sum = 0;

    vector < vector<int>> mat_1 = {{cmat[1][1], cmat[1][2]}, {cmat[2][1], cmat[2][2]}};
    vector < vector<int>> mat_2 = {{cmat[0][1], cmat[0][2]}, {cmat[2][1], cmat[2][2]}};
    vector < vector<int>> mat_3 = {{cmat[0][1], cmat[0][2]}, {cmat[1][1], cmat[1][2]}};
    sum = cmat[0][0]*base_est(mat_1) - cmat[0][1]*base_est(mat_2) + cmat[0][2]*base_est(mat_3);

    return sum;
}

int base_est(vector < vector<int> >& cmat){
